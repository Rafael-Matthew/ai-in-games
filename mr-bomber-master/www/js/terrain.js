...
class Terrain {
  ...
  constructor(initial) {
    ...
    for (let bonus of initial.powerUps) {
      for (let i = 0; i < bonus.count; i++) this.powerUpList.push(bonus.type);
    }
    ...
    for (let y = 0; y < this.height; y++) {
      for (let x = 0; x < this.width; x++) {
        ...
        if (src == "#")
          this.data[y * this.width + x] = { type: TerrainType.PermanentWall };
        else if (src == "-")
          ...
          this.data[y * this.width + x] = { type: TerrainType.Free };
        } else if (src == "%")
          this.data[y * this.width + x] = { type: TerrainType.Rubber };
  ...
  generateSpawn(spawnIndex = -1) {
    if (spawnIndex == -1) {
      ...
      for (let i = 0; i < this.spawns.length; i++) {
        if (!this.spawns[i].busy) indexList.push(i);
      }
  ...
  getCell(x, y) {
    ...
    else return { type: TerrainType.PermanentWall };
  }
  setCell(x, y, cell) {
    ...
    for (let y = 0; y < this.height; y++) {
      for (let x = 0; x < this.width; x++) {
        ...
        if (cell.imageIdx !== undefined) {
          ...
          if (Int.mod(this.time, animateDelay) == 0) {
            ...
            if (cell.imageIdx >= cell.image.length) {
              if (cell.next) this.setCell(x, y, cell.next);
              else cell.imageIdx = 0;
            }
          }
        }
        if (cell.bombTime) {
        ...
        if (cell.type == TerrainType.Bomb) {
          ...
          this.setCell(x, y, { type: TerrainType.Free });
