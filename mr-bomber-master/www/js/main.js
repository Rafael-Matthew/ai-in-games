...
class IdleController {
  ...
  update() {
    this.playerKeys = [];
    // Tidak ada delay, bot bergerak setiap frame

    // Dapatkan posisi bot
    let botSprite = null;
    if (typeof sprites !== "undefined") {
      botSprite = sprites.find((s) => s.controller === this);
    }

    // Cek arah yang bisa dilewati
    const directions = [
      PlayerKeys.Up,
      PlayerKeys.Left,
      PlayerKeys.Right,
    ...
    let cellX = null,
      cellY = null;
    if (botSprite && typeof map !== "undefined") {
      cellX = Math.round(botSprite.x / 16);
      cellY = Math.round(botSprite.y / 16);
      availableDirections = directions.filter((dir) => {
        const delta = dirDelta[dir];
        return map.isWalkable(cellX + delta.x, cellY + delta.y);
      });
    }

    // Cari bomb di sekitar bot
    let bombNearby = false;
    let bombDirections = [];
    if (botSprite && typeof map !== "undefined") {
      for (let dir of directions) {
        const delta = dirDelta[dir];
        const cell = map.getCell(cellX + delta.x, cellY + delta.y);
        if (cell.type === TerrainType.Bomb) {
          bombNearby = true;
          bombDirections.push(dir);
        }
      }
    }

    // If bot is avoiding bomb, keep moving away for a few ticks
    if (this.isAvoidingBomb && this.avoidTicks > 0) {
      if (availableDirections.includes(this.avoidDirection)) {
        this.playerKeys[this.avoidDirection] = true;
      ...
      this.avoidTicks--;
      if (this.avoidTicks === 0) this.isAvoidingBomb = false;
      return;
    }

    // Jika ada bomb di sekitar, pilih arah yang menjauhi bomb
    if (bombNearby && availableDirections.length > 0) {
      // Ambil arah yang tidak mengandung bomb
      let safeDirections = availableDirections.filter(
        (dir) => !bombDirections.includes(dir)
      );
      if (safeDirections.length > 0) {
        this.currentDirection =
          safeDirections[Math.floor(Math.random() * safeDirections.length)];
        this.playerKeys[this.currentDirection] = true;
      } else {
        // Jika tidak ada arah aman, tetap bergerak random
        this.currentDirection =
          availableDirections[
            Math.floor(Math.random() * availableDirections.length)
          ];
    ...
    // Target: kejar bot lain
    if (
      !this.targetSprite ||
      this.targetSprite.isDie ||
      this.targetSprite === botSprite
    ) {
      // Pilih target bot lain yang masih hidup
      let candidates = sprites.filter((s) => s !== botSprite && !s.isDie);
    ...
    // Jika ada target, cari arah menuju target
    if (this.targetSprite && cellX !== null && cellY !== null) {
      const targetX = Math.round(this.targetSprite.x / 16);
      const targetY = Math.round(this.targetSprite.y / 16);
      let moveDir = null;
      if (Math.abs(targetX - cellX) > Math.abs(targetY - cellY)) {
        if (targetX > cellX && availableDirections.includes(PlayerKeys.Right))
          moveDir = PlayerKeys.Right;
        else if (
          targetX < cellX &&
          availableDirections.includes(PlayerKeys.Left)
        )
          moveDir = PlayerKeys.Left;
      } else {
        if (targetY > cellY && availableDirections.includes(PlayerKeys.Down))
          moveDir = PlayerKeys.Down;
        else if (targetY < cellY && availableDirections.includes(PlayerKeys.Up))
          moveDir = PlayerKeys.Up;
      }
      if (moveDir) {
        this.currentDirection = moveDir;
        this.playerKeys[moveDir] = true;
        return;
      }
    }

    // Jika tidak ada target atau tidak bisa ke target, teruskan ke arah saat ini
    if (
      this.currentDirection &&
      availableDirections.includes(this.currentDirection)
    ) {
      this.playerKeys[this.currentDirection] = true;
    } else if (availableDirections.length > 0) {
      // Pilih arah baru jika mentok
      this.currentDirection =
        availableDirections[
          Math.floor(Math.random() * availableDirections.length)
        ];
    ...
    // 5% chance to drop bomb
    // Drop bomb di wall terjauh
    if (
      !this.waitBomb &&
      Math.random() < 0.05 &&
      ...
      // Cari wall terjauh dari posisi bot
      let maxDist = -1,
        bestDir = null;
      for (let dir of availableDirections) {
        ...
        let cx = cellX,
          cy = cellY;
        const delta = dirDelta[dir];
        while (map.isWalkable(cx + delta.x, cy + delta.y)) {
          cx += delta.x;
          cy += delta.y;
          dist++;
        }
      ...
      if (bestDir !== null && maxDist > 0) {
        // Drop bomb di wall terjauh
        this.playerKeys[PlayerKeys.Bomb] = true;
        // Cari titik aman di luar radius bomb
        let radius = botSprite.maxBoom || 1;
        let safeSpots = [];
        for (let dir of availableDirections) {
          let cx = cellX,
            cy = cellY;
          ...
          let steps = 0;
          while (
            map.isWalkable(cx + delta.x, cy + delta.y) &&
            steps < radius + 2
          ) {
            cx += delta.x;
            cy += delta.y;
          ...
          // Pastikan spot di luar radius bomb dan bisa dilewati
          if (steps > radius) {
            safeSpots.push({ x: cx, y: cy, dir: dir });
          }
        }
        // Pilih spot aman terdekat
        if (safeSpots.length > 0) {
          let spot = safeSpots[0];
          // Gerakkan bot ke spot aman
          this.isAvoidingBomb = true;
          this.avoidDirection = spot.dir;
          this.avoidTicks = Math.abs(spot.x - cellX) + Math.abs(spot.y - cellY);
          this.waitBomb = true;
          this.waitBombTicks = 20; // Diam selama 20 update (bisa disesuaikan)
        } else {
          // Jika tidak ada spot aman, tetap menjauh sesuai radius
          this.isAvoidingBomb = true;
          this.avoidDirection = bestDir;
          this.avoidTicks = radius + 1;
          this.waitBomb = true;
          this.waitBombTicks = 20;
        }
        return;
      }
    }
    // Diam sampai bomb meledak
    if (this.waitBomb) {
      this.waitBombTicks--;
      if (this.waitBombTicks <= 0) {
        this.waitBomb = false;
      }
      return;
    }
