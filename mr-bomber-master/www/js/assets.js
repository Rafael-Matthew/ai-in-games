async function loadAssets(scale = 2) {
  ...
  function loadImageStripe(imgElement, x, y, width, height, count, gap) {
    ...
    for (let i = 0; i < count; i++) {
      result.push(
        loadImage(imgElement, x + i * (width + gap), y, width, height)
      );
    }
      ...
      for (let x = 0; x < 5; x++) {
        ...
        for (let index of framesIndex[x]) {
          ...
          newImages.push(
            loadImage(
              imgSpriteBoys,
              (frameX % 13) * spriteWidth,
              Math.floor(frameX / 13) * spriteHeight,
              23,
              23
            )
          );
        ...
        for (let index of framesIndex[x]) {
          ...
          newImages.push(
            loadImage(
              imgSpriteGirl,
              (frameX % 13) * spriteWidth,
              Math.floor(frameX / 13) * (spriteHeight + 2),
              23,
              25
            )
          );
  ...
  async function loadPermanentWall(fire, wall) {
    ...
    for (let i = 0; i < fire.length + 1; i++) {
      ...
      wall.draw(
        ctx,
        x + Int.divRound(width - wall.rect.width, 2),
  ...
  return {
    ...
    monsters: [
      loadMonster(
        ...
        loadImageStripe(imgSprite, 216, 144, 17, 18, 2, 7).concat(
      ...
      loadMonster(
        ...
        loadImageStripe(imgMed3, 89, 122, 32, 32, 4, 1).concat(
    ...
    coin: loadImageStripe(imgMed3, 0, 0, 22, 22, 13, 1).concat(
    ...
    levels: [
      {
        ...
        overlays: [
          {
            ...
            idx: 0,
            animateDelay: 0,
            images: [loadImage(imgMed3, 0, 77, 6 * 8, 44)],
          },
          {
            x: 112,
            y: 30,
        ...
        permanentWalls: await loadPermanentWall(
          fire,
        ...
        overlays: [
          {
            ...
            images: [
              loadImage(
                document.getElementById("FEUILLE_OVERLAY"),
                0,
                0,
                320,
                200
      ...
      {
        background: [
          loadImage(document.getElementById("MICRO"), 0, 0, 320, 200),
        ],
        overlays: [],
        walls: null,
        permanentWalls: await loadPermanentWall(
